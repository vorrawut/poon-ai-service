...
class IntegrationTester:
    ...
    async def test_process_and_store(self, test_case):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/process/text/store",
                json=payload
            ) as response:
                if response.status == 200:
                    result = await response.json()
    ...
    async def verify_storage(self, entry_id: str):
        ...
        try:
            async with self.session.get(f"{self.base_url}/spending/entries/{entry_id}") as response:
                if response.status == 200:
    ...
    async def get_statistics(self):
        ...
        try:
            async with self.session.get(f"{self.base_url}/spending/statistics") as response:
                if response.status == 200:
    ...
    def validate_result(self, test_case, result):
        ...
        if expected_merchant:
            validation["merchant_correct"] = expected_merchant.lower() in actual_merchant.lower()
        else:
        ...
        if expected_category:
            validation["category_correct"] = expected_category.lower() in actual_category.lower()
...
async def main():
    ...
    print(f"🚀 Starting Llama4 AI Service Integration Tests")
    ...
    async with IntegrationTester(BASE_URL) as tester:
        ...
        if not await tester.check_health():
            print(f"❌ Service is not healthy. Please start the AI service first.")
            sys.exit(1)
        ...
        if not ai_available:
            print(f"⚠️ AI service (Llama4) is not available. Some tests may fail.")
            print(f"Please ensure Ollama is running and Llama 3.2 model is installed.")
        ...
        if successful_tests == total_tests:
            ...
            print(f"Check the failed tests and ensure Ollama is properly configured.")
            return 1
        else:
            print(f"\n❌ ALL TESTS FAILED! Please check your configuration.")
            print(f"Ensure the AI service is running and Ollama is properly set up.")
            return 2
