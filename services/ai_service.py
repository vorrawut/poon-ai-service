...
class AIService:
    ...
    async def enhance_nlp_result(
        ...
        try:
            ...
            logger.info(
                f"✅ AI enhancement completed, confidence boosted to {enhanced_result.confidence:.2f}"
            )
            return enhanced_result

        except Exception as e:
            logger.error(f"❌ AI enhancement failed: {e!s}")
    ...
    async def create_spending_entry(
        ...
        try:
            ...
            response = await self._call_openai(
                prompt=prompt, max_tokens=800, temperature=0.1
            )
            ...
            logger.error(f"❌ AI spending entry creation failed: {e!s}")
    ...
    async def enhance_text_parsing(
        ...
        try:
            ...
            response = await self._call_openai(
                prompt=prompt, max_tokens=600, temperature=0.2
            )
    ...
    async def analyze_spending_patterns(
        ...
        try:
            ...
            raise Exception(f"AI analysis failed: {e!s}")
    ...
    async def enhance_spending_entry(self, entry: SpendingEntry) -> SpendingEntry:
        ...
        try:
            ...
            logger.error(f"❌ Entry enhancement failed: {e!s}")
            return entry
    ...
    async def _call_openai(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(f"OpenAI API error on attempt {attempt + 1}: {e!s}")
                if attempt == self.max_retries - 1:
                    raise Exception(f"OpenAI API failed: {e!s}")
