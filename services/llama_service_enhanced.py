...
Focus on practical, culturally appropriate advice for Thai users.""",
    ...
    async def parse_spending_text(
        self, text: str, language: str = "en", context: dict[str, Any] = None
    ) -> NLPResult:
...
Return only valid JSON with the extracted spending information."""
            ...
            response = await self._call_llama(
                user_prompt, self.system_prompts["text_parsing"], max_tokens=600
            )
            ...
            try:
                ...
                logger.info(
                    f"✅ Llama parsing completed with {nlp_result.confidence:.2f} confidence"
                )
                return nlp_result
            ...
            raise Exception(f"Llama parsing failed: {e!s}")
...
Return JSON format with reasoning for any changes made."""
            ...
            response = await self._call_llama(
                user_prompt, self.system_prompts["enhancement"], max_tokens=800
            )
            ...
            if response:
                ...
                logger.info(
                    f"✅ Llama enhancement completed, confidence: {enhanced_result.confidence:.2f}"
                )
...
Return JSON with your best analysis combining OCR and NLP data."""
            ...
            if response:
                ...
                enhanced_result = self._create_comprehensive_result(
                    ocr_result, nlp_result, llama_data
                )
...
Return structured JSON with insights, recommendations, patterns, and anomalies."""
            ...
            response = await self._call_llama(
                user_prompt, self.system_prompts["analysis"], max_tokens=1500
            )

            if response:
                analysis_data = self._parse_llama_response(response)
                analysis = self._create_ai_analysis(
                    entries, analysis_data, analysis_type
                )
    ...
    def _parse_llama_response(self, response: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error processing Llama response: {e!s}")
            return {}
    ...
    def _merge_nlp_results(
        self, original: NLPResult, enhanced_data: dict[str, Any]
    ) -> NLPResult:
        ...
        try:
            ...
            if enhanced_data.get("date"):
                try:
                    enhanced_date = datetime.fromisoformat(
                        enhanced_data["date"].replace("Z", "+00:00")
                    )
                except:
    ...
    def _create_comprehensive_result(
        self, ocr_result: OCRResult, nlp_result: NLPResult, llama_data: dict[str, Any]
    ) -> NLPResult:
        ...
        try:
            ...
            if llama_data.get("date"):
                try:
                    result_date = datetime.fromisoformat(
                        llama_data["date"].replace("Z", "+00:00")
                    )
                except:
                    pass
            ...
            return NLPResult(
                ...
                description=llama_data.get("description")
                or f"Receipt from {llama_data.get('merchant', 'Unknown')}",
                confidence=0.9,  # High confidence for comprehensive processing
                reasoning=llama_data.get(
                    "reasoning", "Comprehensive OCR+NLP+Llama analysis"
                ),
                extraction_details={
                    ...
                    recommendations.append(
                        AIRecommendation(
                            ...
                            potential_savings=float(
                                rec_data.get("potential_savings", 0.0)
                            ),
                        )
                    )
            ...
            return AIAnalysis(
                ...
                insights=[
                    AIInsight(
                        ...
                        description=f"Failed to process analysis: {e!s}",
                        impact="low",
                    )
                ],
                recommendations=[],
