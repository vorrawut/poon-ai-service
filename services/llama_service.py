...
class LlamaService:
    ...
    def __init__(self, settings):
        ...
        self.model = getattr(settings, 'llama_model', 'llama2')  # Will be llama4 when available
        self.max_retries = 3
            ...
            try:
                async with httpx.AsyncClient(timeout=self.request_timeout) as client:
                    payload = {
                        "model": self.model,
                        "prompt": prompt,
                        "system": system_prompt or "You are a helpful financial assistant. Always respond with valid JSON.",
                        "stream": False,
                        "options": {
                            "temperature": 0.1,
                            "top_p": 0.9,
    ...
    async def enhance_nlp_result(self, text: str, nlp_result: NLPResult, language: str = "en") -> NLPResult:
        ...
        try:
            ...
            logger.info(f"✅ Llama4 enhancement completed, confidence boosted to {enhanced_result.confidence:.2f}")
            return enhanced_result
            
        except Exception as e:
            logger.error(f"❌ Llama4 enhancement failed: {str(e)}")
    ...
    async def create_spending_entry(self, ocr_result: OCRResult, nlp_result: NLPResult, language: str = "en") -> NLPResult:
        ...
        try:
            ...
            logger.error(f"❌ Llama4 spending entry creation failed: {str(e)}")
    ...
    async def enhance_text_parsing(self, text: str, nlp_result: NLPResult, language: str = "en", context: Dict[str, Any] = None) -> NLPResult:
        ...
        try:
            ...
            enhanced_result = NLPResult(
                merchant=llama_data.get('merchant') or nlp_result.merchant or 'Voice/Chat Entry',
    ...
    def _parse_analysis_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'insights': [{'title': 'Analysis Error', 'description': 'Failed to parse Llama response', 'impact': 'low'}],
                'recommendations': [],
                'patterns': {},
                'anomalies': []
            }
