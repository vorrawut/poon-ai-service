...
from models.spending_models import SpendingEntry
    ...
    async def initialize(self):
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.execute(
                    "CREATE INDEX IF NOT EXISTS idx_spending_date ON spending_entries(date)"
                )
                await db.execute(
                    "CREATE INDEX IF NOT EXISTS idx_spending_category ON spending_entries(category)"
                )
                await db.execute(
                    "CREATE INDEX IF NOT EXISTS idx_spending_merchant ON spending_entries(merchant)"
                )
                await db.execute(
                    "CREATE INDEX IF NOT EXISTS idx_processing_logs_entry ON processing_logs(entry_id)"
                )
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                async with db.execute(query, params) as cursor:
                    ...
                    for row in rows:
                        ...
                        entry["metadata"] = (
                            json.loads(entry["metadata"]) if entry["metadata"] else {}
                        )
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                async with db.execute(
                    "SELECT * FROM spending_entries WHERE id = ?", (entry_id,)
                ) as cursor:
    ...
    async def delete_spending_entry(self, entry_id: str) -> bool:
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.execute(
                    "DELETE FROM processing_logs WHERE entry_id = ?", (entry_id,)
                )

                # Delete the entry
                cursor = await db.execute(
                    "DELETE FROM spending_entries WHERE id = ?", (entry_id,)
                )
                await db.commit()
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                # Total amount
                async with db.execute(
                    "SELECT SUM(amount) FROM spending_entries"
                ) as cursor:
                    total_amount = (await cursor.fetchone())[0] or 0

                # Average confidence
                async with db.execute(
                    "SELECT AVG(confidence) FROM spending_entries"
                ) as cursor:
