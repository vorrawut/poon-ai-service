...
def validate_image(file: UploadFile) -> bool:
    ...
    try:
        ...
        logger.error(f"Image validation error: {e!s}")
        return False
...
def enhance_for_ocr(image: Image.Image) -> Image.Image:
    ...
    try:
        ...
        logger.error(f"Image enhancement error: {e!s}")
        return image
...
def extract_image_metadata(image_bytes: bytes) -> dict:
    ...
    try:
        ...
        logger.error(f"Metadata extraction error: {e!s}")
        return {}
...
def create_thumbnail(image_bytes: bytes, size: tuple[int, int] = (200, 200)) -> bytes:
    ...
    try:
        ...
        logger.error(f"Thumbnail creation error: {e!s}")
